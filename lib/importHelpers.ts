...
export type JsonIncomeEntry = {
  month: string;
  entries: Array<{ sourceName: string; amount: number }>;
};

export type JsonRecurringExpense = {
  name: string;
  amount: number;
  categoryName: string;
  billingDay: number;
...
type TransactionWriteMode = "upsert" | "create";
...
const pdfSources = new Set(["pdf_smbc", "pdf_jcb", "pdf_saison"]);
...
export async function loadImportData(fileName = "stashy_import.json"): Promise<ImportData> {
...
export async function importAccounts(accounts: JsonAccount[]) {
...
export async function importIncomeEntries(months: JsonIncomeEntry[]) {
...
export async function importRecurringExpenses(expenses: JsonRecurringExpense[]) {
...
function normalizeDebt(raw: JsonDebt, options: { legacyPaidyAppleOverride?: boolean } = {}): JsonDebt {
  if (options.legacyPaidyAppleOverride && (raw.id === "debt_paidy_apple" || raw.cardName.includes("Apple専用"))) {
    return {
      ...raw,
...
export async function importCreditDebts(debts: JsonDebt[], options: { legacyPaidyAppleOverride?: boolean } = {}) {
  console.log("5/7 credit debts: importing debt payoff records...");
  let imported = 0;
  for (const rawDebt of debts) {
    const debt = normalizeDebt(rawDebt, options);
...
export async function dropTransactionIdempotencyIndex() {
  await prisma.$executeRawUnsafe('DROP INDEX IF EXISTS "Transaction_localUserId_date_payee_amountYen_accountId_key"');
}

export async function importTransactions(transactions: JsonTransaction[], mode: TransactionWriteMode = "upsert") {
  ...
  for (const transaction of transactions) {
    ...
    if (mode === "create") {
      ...
      const existing = await prisma.transaction.findFirst({
        where: { localUserId: LOCAL_USER_ID, date, payee, amountYen, accountId },
        select: { id: true },
      });
      if (existing) {
...
export async function importMerchantRules(rules: JsonMerchantRule[]) {
...
export async function upsertDebtPaymentBudgets(data: ImportData) {
...
export async function importAllData(data: ImportData, options: { transactionWriteMode?: TransactionWriteMode; legacyPaidyAppleOverride?: boolean; includeBudgetSupport?: boolean } = {}) {
  resetImportState();
  ...
  await importCreditDebts(data.creditDebts ?? [], { legacyPaidyAppleOverride: options.legacyPaidyAppleOverride });
  await importTransactions(data.transactions ?? [], options.transactionWriteMode ?? "upsert");
  await importMerchantRules(data.merchantRules ?? []);
  if (options.includeBudgetSupport ?? true) await upsertDebtPaymentBudgets(data);
}

export async function wipeImportedData() {
  console.log("Wiping existing imported data...");
  const targets = [
    ["MerchantRule", () => prisma.merchantRule.deleteMany({ where: { localUserId: LOCAL_USER_ID } })],
    ["Transaction", () => prisma.transaction.deleteMany({ where: { localUserId: LOCAL_USER_ID } })],
    ["RecurringExpense", () => prisma.recurringExpense.deleteMany({ where: { localUserId: LOCAL_USER_ID } })],
    ["IncomeEntry", () => prisma.incomeEntry.deleteMany({ where: { localUserId: LOCAL_USER_ID } })],
    ["CreditDebt", () => prisma.creditDebt.deleteMany({ where: { localUserId: LOCAL_USER_ID } })],
    ["Account", () => prisma.account.deleteMany({ where: { localUserId: LOCAL_USER_ID } })],
  ] as const;

  for (const [name, action] of targets) {
    const result = await action();
    console.log(`  Deleted ${result.count} ${name} records.`);
  }
}

export async function dedupePdfCsvTransactions() {
  console.log("Deduplication pass: resolving csv/pdf transaction overlaps...");
  const groups = await prisma.$queryRaw<Array<{ date: Date; accountId: string; amountYen: number; count: bigint }>>`
    ...
    HAVING COUNT(*) > 1
    ORDER BY "date", "accountId", "amountYen"
  `;

  let deleted = 0;
  for (const group of groups) {
    const rows = await prisma.transaction.findMany({
      where: { localUserId: LOCAL_USER_ID, date: group.date, accountId: group.accountId, amountYen: group.amountYen },
      select: { id: true, source: true, payee: true, date: true, amountYen: true },
      orderBy: { createdAt: "asc" },
    });
    const hasPdf = rows.some((row) => pdfSources.has(row.source));
    const csvRows = rows.filter((row) => row.source === "csv");
    if (!hasPdf || csvRows.length === 0) continue;

    for (const row of csvRows) {
      await prisma.transaction.delete({ where: { id: row.id } });
      deleted += 1;
      const date = row.date.toISOString().slice(0, 10);
      console.log(`  Deleted csv duplicate ${date} account=${group.accountId} amount=¥${row.amountYen.toLocaleString("ja-JP")} payee=${row.payee}`);
    }
  }
  console.log(`  Deduplication complete. Deleted ${deleted} csv duplicates.`);
  return deleted;
}

export async function printVerification() {
  console.log("\nVerification checks:");
  const [accountsCount, transactions, debts, paidyInstallments, incomeEntries, recurringCount, paidyApple] = await Promise.all([
    ...
    prisma.creditDebt.findMany({ where: { localUserId: LOCAL_USER_ID, type: "installment", cardName: { contains: "Paidy" } }, select: { monthlyPaymentYen: true } }),
    ...
    prisma.creditDebt.findFirst({ where: { localUserId: LOCAL_USER_ID, cardName: { contains: "Apple" } }, select: { cardName: true, currentBalanceYen: true, monthlyPaymentYen: true, totalInstallments: true, installmentsPaid: true } }),
  ]);
  ...
  const riboTotalOutstanding = debts.filter((debt) => debt.type === "revolving" && /SMBC|三井|JCB|セゾン|Saison/i.test(debt.cardName)).reduce((total, debt) => total + debt.currentBalanceYen, 0);
...
export async function assertV2Import() {
  console.log("\nV2 assertion checks:");
  const [transactionCount, incomeEntries, debts] = await Promise.all([
    prisma.transaction.count({ where: { localUserId: LOCAL_USER_ID } }),
    prisma.incomeEntry.findMany({ where: { localUserId: LOCAL_USER_ID }, select: { month: true, amountYen: true } }),
    prisma.creditDebt.findMany({ where: { localUserId: LOCAL_USER_ID }, select: { currentBalanceYen: true } }),
  ]);

  const incomeByMonth = new Map<string, number>();
  for (const entry of incomeEntries) incomeByMonth.set(entry.month, (incomeByMonth.get(entry.month) ?? 0) + entry.amountYen);
  const expectedIncome = new Map([
    ["2026-01", 809_305],
    ["2026-02", 853_615],
    ["2026-03", 851_786],
    ["2026-04", 907_820],
  ...
  const assertEqual = (label: string, actual: number, expected: number) => {
    if (actual !== expected) failures.push(`${label}: expected ${expected.toLocaleString("ja-JP")}, got ${actual.toLocaleString("ja-JP")}`);
    else console.log(`  ✓ ${label}: ${actual.toLocaleString("ja-JP")}`);
  };

  assertEqual("Total transactions", transactionCount, 326);
  assertEqual("Income entries", incomeEntries.length, 30);
  assertEqual("Income months", incomeByMonth.size, 5);
  for (const [month, expected] of expectedIncome) assertEqual(`${month} income`, incomeByMonth.get(month) ?? 0, expected);
  assertEqual("Total debts", debts.length, 7);
  assertEqual("Grand total outstanding debt", debts.reduce((total, debt) => total + debt.currentBalanceYen, 0), 1_232_315);

  if (failures.length > 0) {
    console.error("\nV2 import assertions failed:");
    for (const failure of failures) console.error(`  ✗ ${failure}`);
    throw new Error("V2 import verification failed.");
  }
}

export function printManualStepsReminder() {
  console.log("\nMANUAL STEPS REQUIRED:");
  console.log("1. Set current balance for ゆうちょ銀行 (acc_yucho)");
  console.log("2. Set current balance for ソニー銀行 (acc_sony)");
  console.log("3. Set current balance for 楽天カード (acc_rakuten)");
  console.log("4. Verify PayPay ¥173,000 and Mercari ¥28,000 balances are current");
  console.log("5. Note: Jan salary was ¥560,898 not ¥594,672 — each month varies slightly");
}

export async function disconnectImportPrisma() {
